...
def getHistData(startDate, duration):
    ...
    #Querry the data
    print(cursor.execute("SELECT datetime(timestamp, 'localtime') as timestamp, temperature, humidity FROM weather WHERE timestamp >= \'{startDate}\' AND timestamp <= \'{endDate}\' ORDER BY timestamp DESC;"))
